...
// calculating the fuel consumption according to the given formula
-(float)calcFuelConsumption:(float)timeDiff taxiObject:(Taxi *)taxi{
    return timeDiff * (([self absFloat:tanf([taxi getInitialVelocity]*timeDiff)]) + [self absFloat:cosf([taxi getInitialVelocity])]);
}
...
-(void)calcCost:(float) difference taxiObject:(Taxi *)taxi;{
    if(difference <=12){
        ...
        if( fuelConsumption < [taxi getFuelTankLevel] ){ // if tank has enough fuel
            ...
            printf("\n****Income generated by current trip = %f  ****\n",cost);
            ...
            printf("\nNot enough fuel. Please fill the tank. Penalty of $100\n");
            [taxi setNetIncomeGenerated:([taxi getNetIncomeGenerated] - 100)];
        }
